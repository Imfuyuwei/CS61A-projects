...
def free_bacon(score):
    ...
    tens_digit = score // 10
    points = 2 + abs(units_digit-tens_digit)
    return points
    # END PROBLEM 2
        ...
        if who == 0:
            if score0 - previous_score > previous_high:
                ...
                if current_high == 1:
                    ...
                    print(str(current_high) + " points! That's the biggest gain yet for Player " + str(who))
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    i, k, max_score = 1, 1, averaged_roll_dice(1, dice)
    while i < 10:
        if max_score < averaged_roll_dice(i+1, dice):
            max_score = averaged_roll_dice(i+1, dice)
            k = i+1
            i += 1
        else:
            i += 1
