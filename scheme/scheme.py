...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong number of arguments were passed into the function')
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    if expressions is not nil:
        if expressions.second is not nil:
            answer = scheme_eval(expressions.first, env)
        else:
            answer = scheme_eval(expressions.first, env, True)
        if scheme_falsep(answer) or expressions.second is nil:
            return answer
        else:
...
def do_define_macro(expressions, env):
    ...
    def do_macro_form(expressions, env):
        ...
        check_formals(formals)
        body = expressions.second
        return MacroProcedure(formals, body, env)
    check_form(expressions, 2)
    target = expressions.first
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        env.define(target.first, do_macro_form(Pair(target.second, expressions.second), env))
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
        ...
        while isinstance(result, Thunk):
            ...
            if (scheme_symbolp(first) and first in SPECIAL_FORMS):
                ...
                if isinstance(procedure, MacroProcedure):
                    args = rest
                    new_expr = procedure.apply_macro(args, env)  
                    result = optimized_eval(new_expr, env)  
                else:
                    args = rest.map(lambda operand: scheme_eval(operand, env))
