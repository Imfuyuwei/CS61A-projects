...
(odd? 1 2)
; expect Error: wrong number of arguments were passed into the function
...
(cond
  ((= x 1) #t)
  ((= x 2) #f)
  (else "x is not anything above"))
; expect "x is not anything above"
...
(no-repeat (list 5 4 3 3 6 9 2 6 5))
; expect (5 4 3 6 9 2)

(define (circulation) (circulation))
(cond (false (circulation))
      (1))
; expect 1

(define x 7)
(let ((x 1)
           (y ((lambda (a) (+ a (or #f 4))) x))) 
       ((lambda (a b) (* a b)) x y))
; expect 11
...
(list-of (factorial x 1) for x in '(1 2 3 4 5) if (even? x))
; expect (2 24)
