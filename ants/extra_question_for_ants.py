    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.can_contain(insect) and insect.container == False:
                    self.ant.contain_ant(insect)
                elif self.ant.container == False and insect.can_contain(self.ant):
                    insect.place = self
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        inspected_place = self.place
        distance = 0
        while inspected_place != hive:
            if len(inspected_place.bees) > 0:
                if distance >= self.min_range and distance <= self.max_range:
                    return random_or_none(inspected_place.bees)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            self.place.bees = [bee for bee in bees_copy if bee.armor > 0]
            self.place.remove_insect(self)
        # END Problem 5
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.TrueQueen:
            ...
            inspected_place = self.place.exit
            while inspected_place.exit is not None:
                if inspected_place.ant:
                    if inspected_place.ant not in self.doubled:
                        inspected_place.ant.damage *= 2
                        self.doubled.append(inspected_place.ant)
                    if inspected_place.ant.container:
                        if inspected_place.ant.ant:
                            if inspected_place.ant.ant not in self.doubled:
                                inspected_place.ant.ant.damage *= 2
                                self.doubled.append(inspected_place.ant.ant)
                inspected_place = inspected_place.exit
        else:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    original_effect = bee.action
    other_effect = effect(bee.action)
