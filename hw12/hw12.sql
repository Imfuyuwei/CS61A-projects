...
CREATE TABLE by_height AS
  ...
  FROM dogs, parents 
  WHERE dogs.name = parents.parent ORDER BY -dogs.height;

-- Filling out this helper table is optional
CREATE TABLE siblings AS
  SELECT a.child AS sibiling1, b.child AS sibiling2 
  FROM parents AS a, parents AS b 
  WHERE a.parent = b.parent AND a.child < b.child;

-- Sentences about siblings that are the same size
CREATE TABLE sentences AS
  SELECT a.sibiling1 || " and " || a.sibiling2 || " are " || b.size || " siblings"
  FROM siblings AS a, size_of_dogs AS b, size_of_dogs AS c
  WHERE a.sibiling1 = b.name AND a.sibiling2 = c.name AND b.size = c.size;
...
SELECT stacks_helper.dogs || ", " || dogs.name, stacks_helper.stack_height + dogs.height, dogs.height 
FROM stacks_helper, dogs 
WHERE dogs.height > stacks_helper.last_height;
